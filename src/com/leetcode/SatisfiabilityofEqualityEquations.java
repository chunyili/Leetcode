 ...
 * has length 4 and takes one of two different forms: "a==b" or "a!=b".  Here, a and b are lowercase letters (not necessarily
...
public class SatisfiabilityofEqualityEquations {
    public boolean equationsPossible(String[] equations) {
        ...
        for (String e : equations) {
            if (e.charAt(1) == '=') {
                int x = find(parent, e.charAt(0) - 'a');
                int y = find(parent, e.charAt(3) - 'a');
                if (x != y) {
                    parent[x] = y;
                }
            }
...
// Second pass all inequality, O(N)
//
// Overall O(N)